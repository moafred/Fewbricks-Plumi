...
import type { GamePhase, AnswerResult, ArdoiseCalculItem, MathOperation } from '@plumi/shared';
import { generateArdoiseCalculItems } from '@plumi/shared';

export const useArdoiseCalculStore = defineStore('ardoise-calcul', () => {
  // --- State ---
  const phase = ref<GamePhase>('discovery');
  const items = ref<ArdoiseCalculItem[]>([]);
  ...
  function startGame(
    ...
    clearTimer();
    items.value = generateArdoiseCalculItems(count, {
  ...
  function submitAnswer(choice: string) {
    ...
    const correct = choice === String(item.correctAnswer);
    lastResult.value = correct ? 'correct' : 'incorrect';
    selectedChoice.value = choice;
    correctForm.value = String(item.correctAnswer);
